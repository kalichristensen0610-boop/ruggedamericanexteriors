...
export function EstimateForm({compact=false}:{compact?:boolean}){
  ...
  return <form onSubmit={submit} className="mt-6 grid gap-4" encType="multipart/form-data">
    <div className={compact?'grid gap-4 sm:grid-cols-2':'grid gap-4 md:grid-cols-2'}>
      <Field label="Name" name="name" autoComplete="name" required/>
      <Field label="Phone" name="phone" type="tel" autoComplete="tel" required/>
      <Field label="Email" name="email" type="email" autoComplete="email" required/>
      <Field label="Property address or ZIP" name="location" autoComplete="street-address" required/>
    </div>
    ...
    <label className="grid gap-1 text-sm font-bold">Project details
      <textarea name="details" required minLength={10} maxLength={5000} rows={compact?3:5} className="focus-ring border border-ink/30 bg-white p-3"/>
    </label>
